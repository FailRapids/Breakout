...
export(float,0,5) var physics_speed = 2

export(Vector2) var gravity = Vector2(0,20)
...
func _physics_process(delta):
	...
	var status = state_stack.front().physics_update(delta*physics_speed)
	if status in Commands: call_command(status)
	if status in States: self.active_state = self.States[status]
...
func is_on_ground():
	...
	if $Rays/Right.is_colliding():
		return true 
	if $Rays/Center.is_colliding():
		return true
	return false  
		
func get_move_direction():
	#Overide with movement logic
